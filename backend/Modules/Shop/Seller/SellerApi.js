...
module.exports = (shopProductCollection) => {
    ...
    sellerApiRouter.patch('/update-shop-products/:id', async (req, res) => {
        const id = req.params.id;
        ...
        const doc = {
              $set: {
                    publishStatus : updatedData.updatedData,
                    adminFeedback: updatedData.adminFeedback,
              }
        };
        const result = await shopProductCollection.updateOne(filter, doc, options);
        res.send(result)
    })
